...
cf8.exe 2>/dev/null >/dev/null service $NAME || cf8.exe create-service hana hdi-shared $NAME
while true; do
    STATUS="$(cf8.exe 2>/dev/null service $NAME | grep status: | head -n 1)"
    echo $STATUS
...
cf8.exe create-service-key $NAME $NAME-key
echo "SECRET_HEADER = $SECRET_HEADER">./scripts/Log.txt
node "$(dirname "$0")/format-kyma-secret.js" -- "$(echo "$SECRET_HEADER")" "$(cf8.exe service-key $NAME $NAME-key)" | kubectl apply -f -
echo
